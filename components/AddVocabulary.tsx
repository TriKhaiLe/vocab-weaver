...
import { generateMemorableSentence, getWordLemma, translateToVietnamese, checkWordSpelling } from '../services/geminiService';
...
export const AddVocabulary: React.FC<AddVocabularyProps> = ({ addVocabularyEntry }) => {
  ...
  const [translatedGenerated, setTranslatedGenerated] = useState<string | null>(null);
  ...
  const handleWordClick = async (word: string, index: number) => {
    ...
    if (cleanedWord) {
      ...
      try {
        ...
        if (!isValid) {
          setSpellingError(`'${cleanedWord}' might be misspelled or not a valid English word.`);
  ...
  const handleSuggestionClick = (suggestion: string) => {
    ...
    const updatedWord = originalWordWithPunctuation.replace(originalCleanedWord, suggestion);
  ...
  const handleGenerate = async () => {
    ...
    setTranslatedGenerated(null);
    try {
      const result = await generateMemorableSentence(selectedWord);
  ...
  const handleSave = async (withGeneratedSentence: boolean) => {
    ...
    if (withGeneratedSentence && !generatedSentence) {
        setError('Generated sentence is missing. Please generate one before saving.');
        return;
    }
    ...
    try {
      ...
      const newEntry: Omit<VocabularyEntry, 'id'> = {
        ...
        ...(translatedInput && { originalSentenceTranslation: translatedInput }),
  ...
  const TranslateIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 21l-5.25-5.25m0 0L10.5 10.5m-5.25 5.25h15m-15 0l5.25 5.25M3.75 4.5h15m-15 0l5.25 5.25M3.75 4.5l5.25-5.25" />
    </svg>
  );
    ...
    <div className="space-y-8 animate-slide-up">
      ...
      <div className="p-6 bg-dark-card rounded-lg border border-dark-border">
        <div className="flex justify-between items-center mb-3">
          <label htmlFor="sentence-input" className="block text-lg font-semibold text-brand-primary">
        ...
        <div className="relative">
          ...
          <div className={`absolute bottom-2 right-3 text-xs pointer-events-none ${
              sentence.length >= characterLimit ? 'text-red-400' : 
              sentence.length > characterLimit - 50 ? 'text-yellow-400' : 'text-medium-text'
          }`}>
      ...
      {selectedWord && (
         <div className="p-6 bg-dark-card rounded-lg border border-dark-border text-center">
          <h2 className="text-lg font-semibold mb-4 text-brand-primary">
            Step 3 (Optional): Weave a memorable sentence for "{selectedWord}"
          </h2>
          <button
          ...
          >
            {isLoading && !generatedSentence ? <Spinner /> : 'Generate'}
          </button>
        </div>
      )}
      ...
      {generatedSentence && (
        <div className="p-6 bg-dark-card rounded-lg border border-dark-border animate-fade-in">
          <div className="flex justify-between items-center mb-3">
             <h3 className="text-lg font-semibold text-brand-primary">Generated Sentence:</h3>
              <button 
